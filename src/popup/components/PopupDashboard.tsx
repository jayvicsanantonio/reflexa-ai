/**
 * Full dashboard view for the popup
 * Shows reflections, stats, and export functionality
 */
...
import type { Reflection, StreakData, CalmStats as CalmStatsType } from '../../types';
import { PRIVACY_NOTICE } from '../../constants';

interface PopupDashboardProps {
  reflections: Reflection[];
  streakData: StreakData;
  calmStats: CalmStatsType;
  onDelete: (id: string) => void;
  onStreakIncrease?: () => void;
}

/**
 * Dashboard component showing full reflection history and stats
 * Memoized to prevent unnecessary re-renders
 */
const PopupDashboardComponent: React.FC<PopupDashboardProps> = ({
  reflections,
  streakData,
  calmStats,
  onDelete,
  ...
  useEffect(() => {
    const checkFirstLaunch = async () => {
      try {
        const result = await chrome.storage.local.get('reflexa:firstLaunch');
        if (!result['reflexa:firstLaunch']) {
          setShowPrivacyNotice(true);
          await chrome.storage.local.set({ 'reflexa:firstLaunch': true });
        }
      } catch (error) {
        // Error is logged but doesn't block functionality
...
export const PopupDashboard = React.memo(PopupDashboardComponent);
